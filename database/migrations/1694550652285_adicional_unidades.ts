...
/**
 * Classe de migração para criar a tabela 'adicional_unidade'.
 *
 * Esta migração verifica se a tabela 'adicional_unidade' já existe no banco de dados.
...
export default class extends BaseSchema {
   ...
   * @type {string}
   */
  protected tableName: string = 'adicional_unidade'

  /**
   * Método 'up' da migração.
   * Cria a tabela 'adicional_unidade' se ela não existir.
   * 
   * @public
    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          table.integer('adicional_id').notNullable().unsigned().references('id').inTable('cobranca.adicional').onDelete('CASCADE').onUpdate('CASCADE')
          ...
          table.integer('carencia_novo').notNullable().defaultTo(0)
          table.integer('carencia_atraso').notNullable().defaultTo(0)
   ...
   * Exclui a tabela 'adicional_unidade' se ela existir.
   *
   * @public
   * @returns {Promise<void>}
   */
